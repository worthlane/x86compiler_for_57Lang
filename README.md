...
#### Грамматика
Так как код распознается с помощью [рекурсивного спуска](https://ru.wikipedia.org/wiki/Метод_рекурсивного_спуска), возможно будет полезна грамматика, по которому считывается заданный текст.
```
...
Затем синтаксический парсер проходится по лексемам при помощи алгоритма рекурсивного спуска, и на его основе строит AST-дерево программы, характеризующее операции в ней.
...
### Middleend
На второй стадии программа проходится по AST-дереву и оптимизирует всевозможные операции, результат которых извествен заранее. Тогда исполняемому коду не нужно будет тратить время на рассчет итак известных параметров.
...
Так как не все команды в IR могут иметь свои аналоги, я написал стандартную библиотеку, содержащую функций, заменяющих отсутствующие команды.
...
Источники: [skyfree](http://www.skyfree.org/linux/references/ELF_Format.pdf), [tchajed](https://github.com/tchajed/minimal-elf), [scratchpad](https://scratchpad.avikdas.com/elf-explanation/elf-explanation.html)  
