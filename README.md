...
Третий заголовок отвечает за переменные, отвечающие за стандартную библиотеку. ```memsiz``` здесь постоянный, а начинаются данные после всего кода, на адресе кратном ```0x1000```.
