...
Операции с переменными будем проводить через ```XMM``` регистры, для того, чтобы сохранять дробные значения в переменных.
...
#### Оптимизация IR
Получив наполненный IR можно оптимизировать его с помощью линейных проходов, находя нейтральные комбинации инструкций. Я добавил оптимизацию, заменяющую последовательные ```push``` и ```pop``` с одинаковыми аргументами на 2 инструкции ```nop``` (которые в случае IR не делают ничего и занимают 0 байт в любой кодировке).
