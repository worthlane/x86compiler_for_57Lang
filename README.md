...
* ```WORD``` - ожидание токена ```WORD```;
* ```{ A | B | C | ... }``` - ожидание одной из ситуаций: ```A```, ```B``` или ```C```;
* ```[WORD]``` - не обязательный токен;
* ```A*``` - ожидание ситуации ```A```, повторяющейся 0 либо больше раз.
...
После того, как мы получили IR из AST-дерева, нужно транслировать его в инструкции под одну из архитектур. В этом случае будем транслировать в elf-файл для x86-64. Так как наш IR фактически состоит из некоторых "абстрактных" инструкций, нам нужно каждую из инструкций, содержащихся в IR задать в качестве одной или нескольких ассемблерных команд. Я задаю перевод этим инструкциям в файле ```src/backend/compiler/instructions.h```. Здесь используется кодогенерация, благодаря которой мы можем выводить инструкции как в ```.s``` файл для nasm (такой файл создается при флаге ```-S``` во время компиляции), так и сразу в ELF-файл.
...
В ```std_lib/stdlib.s``` содержатся реализация таких инструкций как ```in```, ```out```, ```hlt```.
